...
function getPathsObject(nodeSrc, options) {
  ...
  const inPath = path.resolve('./dist/', nodeSrc);
  // const outDir = path.dirname(path.resolve('./', options.outputDir, nodeSrc));
  const outDir = path.dirname(
    path.join(process.cwd(), options.outputDir, nodeSrc)
  );
  const relativeDir = path.dirname(path.join('/', nodeSrc));
  const filename = path.basename(inPath);
  // const outUrl = path.relative('.', path.join(outDir, filename));
  const outUrl = path.join(relativeDir, filename);
  // console.log({ outDir, filename, outUrl, relativeDir, nodeSrc });
...
function getProcessingPathsForNode(node, options) {
  ...
  const fullPath = path.resolve('./dist/', removedDomainSlash);
...
function getRelativePath(p) {
  // console.log({ p, relativePath: path.relative('./dist/', p) });
...
function ensureOutDirExists(outDir) {
  // mkdirp(path.join('./dist', getRelativePath(outDir)));
  const dirPath = path.join('./dist', outDir);
  // console.log(`ensuring directory ${dirPath} exists`);
  mkdirp(dirPath);
}
...
async function resize(size, paths, options, meta = null) {
  ...
  // console.log({ outPath, outUrl, outPathWebp });
