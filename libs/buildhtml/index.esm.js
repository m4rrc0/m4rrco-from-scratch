...
async function compile(srcPath, options) {
  const { htmlOnly } = options
  ...
  try {
    ...
    let clientSource = htmlOnly ? null : source
    ...
    if (isSvelte) {
      ...
      clientSource = clientApp && clientApp.js.code
      ssrSource = ssrApp.js.code
    }
    ...
    const destPath = srcPath
      .replace(/^src\//, 'dist/')
      .replace(/.svelte$/, '.js')
    const destPathSsr = destPath.replace(/^dist\//, 'build/')

    if (clientSource) {
      // Create all ancestor directories for this file
      await fs.mkdir(path.dirname(destPath), { recursive: true })
      await fs.writeFile(destPath, clientSource)

      console.info(`Svelte compiled ${destPath}`)
    }

    await fs.mkdir(path.dirname(destPathSsr), { recursive: true })
    await fs.writeFile(destPathSsr, ssrSource)

    console.info(`Svelte compiled temporary file ${destPathSsr}`)
